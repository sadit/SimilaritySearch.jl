...
begin
	function fill_something!(m_, dim)
		m = reshape(m_, (dim, dim))
		for i in 0:rand(1:dim)
			m[rand(1:dim), rand(1:dim)] = 1.0
		end

		for i in 2:dim-1, j in 2:dim-1
	...
	n = 10_000
	dim = 7
	A = zeros(Float32, dim*dim, n)
	for m in eachcol(A)
		fill_something!(m, dim)
	end
	X = MatrixDatabase(A)
end

# ╔═╡ 516776f4-7eeb-426a-9d2e-e91a0427b067
...
index = ExhaustiveSearch(; dist=SqL2Distance(), db=X);
