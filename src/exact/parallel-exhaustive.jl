...
function search(pex::ParallelExhaustiveSearch, ctx::GenericContext, q, res::AbstractKnnQueue)
    ...
    # NOTE: forced to :dynamic (never :static/:greedy, regardless of ctx.scheduler) because
    ...
    # batch only ever touches its own @batchid()-indexed column), so :dynamic's migratable
    # tasks are safe here regardless of the global scheduler; the schedule is named
    # explicitly rather than deferred to :default, which is whatever Julia happens to pick.
    # `ctx.scheduler === :sequential` is still honored (falls through to @BATCHES's own
    # single-batch fast path), since that is an explicit request to disable threading
    # entirely, not a scheduler *kind* choice.
    @BATCHES minbatch scheduler=(ctx.scheduler === :sequential ? :sequential : :dynamic) begin
    @BEGIN
