...
function shrinking_neighborhood_search{T, R <: Result}(nsearch::ShrinkingNeighborhoodSearch, index::LocalSearchIndex{T}, q::T, res::R, tabu::BitVector, candidates::SlugKnnResult)
...
function search{T, R <: Result}(nsearch::ShrinkingNeighborhoodSearch, index::LocalSearchIndex{T}, q::T, res::R)
    n = length(index.db)
    tabu = falses(n)
    candidates = SlugKnnResult(nsearch.candidates_size)
