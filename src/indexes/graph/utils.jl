...
function estimate_knearest{T, D, R, R2}(db::Vector{T}, dist::D, choosek::Int, from::Int, q::T, tabu::BitVector, res::R, near::R2)::R2
    n::Int32 = length(db)
...
function estimate_knearest{T, D, R}(db::Vector{T}, dist::D, choosek::Int, from::Int, q::T, tabu::BitVector, res::R)::KnnResult
    near = KnnResult(choosek)
    estimate_knearest(db, dist, choosek, from, q, tabu, res, near)
end

function estimate_from_oracle{T, D, R}(index::LocalSearchIndex{T,D}, q::T, beam::SlugKnnResult, tabu::BitVector, res::R, oracle::Function)
