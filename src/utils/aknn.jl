...
"""
Finds k-nearest-neighbors for all items in the gives dataset.

This function is optimized for the SearchGraph similarity-index
"""
function allknn(index::SearchGraph{T}, dist::Function, X::AbstractVector{T}; k::Int=1, bsize::Int=4) where T
    n = length(X)
    A = [KnnResult(k) for i in 1:n]
    # TODO: Test caching the distance function
    for i in 1:n
